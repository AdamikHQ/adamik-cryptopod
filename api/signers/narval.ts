...
const run = async () => {
  ...
  console.log("requesting wallet create access token");
  ...
  console.log("adding wallet to entity store");
  await addAccount({
    account: {
      ...account,
  ...
  console.log(`wallet created with address ${account.address}`);
};
...
export class NarvalSigner implements Signer {
  ...
  public async registerNewWallet(chainId: string): Promise<string> {
    await run();
    return Promise.resolve("narval_wallet_id");
  }
}
